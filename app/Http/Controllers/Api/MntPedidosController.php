...
class MntPedidosController extends Controller
{
    ...
    public function index(Request $request){
        try {
            // Filtrado básico y ordenado por fecha
            $query = MntPedidos::with([
                'detallePedido.producto', 
                'cliente'
            ]);
            
            // Filtrar por fecha si existe un parámetro de fecha en la solicitud
            if ($request->has('fecha_pedido')) {
                $query->whereDate('fecha_pedido', $request->fecha_pedido);
            }
    
            // Ordenar por fecha de pedido (puedes personalizar esto)
            $pedidos = $query->orderBy('fecha_pedido', 'desc')->get();
    
            // Validar si el array de pedidos está vacío
            if ($pedidos->isEmpty()) {
                return ApiResponse::success('No hay pedidos registrados', 200, []);
            }
    
            return ApiResponse::success('Lista de pedidos', 200, $pedidos);
        } catch (\Exception $e) {
